...
| Field KYC untuk pelaporan goAML | `customers.gender`, `nationality`, `addressType`/`addressCountry`/`addressProvince`/`addressCity`/`addressDistrict`/`addressPostalCode`, `npwp` (migrasi `0031`) | Ditambahkan agar profil nasabah memenuhi field wajib `t_person_my_client` skema XML goAML (gender, nationality1, residence, t_address, tax_reg_number) sebelum generator laporan LTKT/LTKM dibangun. Wajib diisi untuk nasabah **baru** (`customerInput`/`customerUpdateInput`); nasabah lama tetap dapat dipakai dengan kolom ini `NULL` sampai dilengkapi lewat Edit. `last_name` goAML dipetakan dari `fullName` utuh (tanpa split first/last — konvensi umum untuk nama Indonesia yang sering satu kata); `source_of_wealth` goAML dipetakan dari `sourceOfFunds`. |
