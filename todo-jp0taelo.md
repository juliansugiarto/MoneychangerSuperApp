...
- [x] Tambahkan ekspor CSV SIPESAT (PPATK) — kartu baru di halaman **Pelaporan Regulator** (`RegulatoryReportingAdvanced.tsx`, Controller ke atas), membangun file CSV siap unggah manual ke sipesat.ppatk.go.id, **tidak pernah** submit otomatis. Header kolom dan format pipe-delimited persis sesuai contoh resmi PPATK yang diberikan user (`IDPJK|Kode Nasabah|Nama Nasabah|Tempat Lahir|Tanggal Lahir|Alamat|No.KTP|No.Identitas Lain|No.CIF|No.NPWP`); penamaan file (`SIPESAT_IDPJK_IN/TW_...`) sesuai Buku Petunjuk Penggunaan SIPESAT v.3.1 yang diberikan user. `company_profile.sipesatIdPjk` (migrasi `0030`) baru untuk menyimpan ID PJK, diedit di Profil Perusahaan. Dua jenis ekspor: Data Initial (semua nasabah live) dan Data Triwulan (nasabah dibuat/diperbarui pada periode — **asumsi terbaik**, bukan aturan yang eksplisit didokumentasikan, dilabeli demikian di UI+panduan). **Keterbatasan diketahui dan didokumentasikan**: kolom No.NPWP selalu kosong karena `customers` belum membedakan individu/korporasi atau menyimpan NPWP per nasabah. Diuji di `server/sipesatExport.test.ts` (6 test, termasuk reproduksi byte-per-byte dari 2 baris contoh resmi PPATK yang diberikan user).
