...
- [x] Tambahkan test integrasi untuk validasi IDR-ditolak-sebagai-baris dan alasan mismatch nama rekening di `createTransaction` (`server/createTransactionValidation.test.ts`) — kedua guard ini throw sebelum `db.transaction` dibuka, jadi hanya perlu mock `db.select` level atas (bukan `tx` penuh); 6 skenario termasuk perbandingan nama case-insensitive dan validasi tiga field rekening lawan wajib bersamaan.
...
- [x] Ambang underlying otomatis (`exchange_transactions.thresholdReason`, migrasi `0028`) — `createTransaction` memaksa `underlyingRequired = true` begitu nilai setara USD (kurs referensi BI) mencapai 10.000, walau kotak centang tidak dicentang manual; `thresholdReason` wajib diisi terpisah dari `underlyingNotes`. Underlying kini terdiri dari 3 dokumen wajib (Formulir Underlying/Surat Pernyataan/Invoice) — `submitTransaction` menolak pengiriman sampai ketiganya tersimpan. Diuji di `server/createTransactionValidation.test.ts`.
- [x] Tambahkan fitur Transaksi Mencurigakan (TKM) — checklist indikator PPATK (`shared/suspiciousTransactionIndicators.ts`, 3 kategori: perilaku nasabah, profil transaksi, khusus KUPVA BB) muncul saat toggle TKM dicentang, minimal satu indikator wajib, tervalidasi ulang di server. Data **internal-only**: tidak pernah tercetak di kwitansi atau ekspor CSV (larangan tipping-off), hanya lencana **TKM** di Daftar Transaksi. Menandai TKM otomatis memaksa `requiresReview = true`. Diuji di `server/createTransactionValidation.test.ts`.
- [x] Deploy putaran 2026-09-01 kelima ke produksi (commit `a66aba3`, termasuk migrasi `0028`) — backup bersih, migrasi diterapkan, build sukses, pm2 stabil, smoke check 200. **Belum diverifikasi visual di browser** (form TKM & underlying 3-dokumen baru).
...
- [x] Tambahkan ekspor CSV SIPESAT (PPATK) — kartu baru di halaman **Pelaporan Regulator** (`RegulatoryReportingAdvanced.tsx`, Controller ke atas), membangun file CSV siap unggah manual ke sipesat.ppatk.go.id, **tidak pernah** submit otomatis. Header kolom dan format pipe-delimited persis sesuai contoh resmi PPATK yang diberikan user (`IDPJK|Kode Nasabah|Nama Nasabah|Tempat Lahir|Tanggal Lahir|Alamat|No.KTP|No.Identitas Lain|No.CIF|No.NPWP`); penamaan file (`SIPESAT_IDPJK_IN/TW_...`) sesuai Buku Petunjuk Penggunaan SIPESAT v.3.1. `company_profile.sipesatIdPjk` (migrasi `0030`) baru untuk menyimpan ID PJK, diedit di Profil Perusahaan.
  - Cakupan dua jenis ekspor **dikonfirmasi ulang** terhadap regulasi asli Peraturan Kepala PPATK Nomor PER-02/1.02/PPATK/02/2014 (dipelajari user, PDF `AturanPelaporanSIPESAT.pdf`) setelah rilis pertama sempat memakai asumsi yang salah: **Data Initial** = seluruh nasabah live *termasuk yang sudah ditutup* (Pasal 13); **Data Triwulan** = *hanya nasabah baru* yang `createdAt`-nya jatuh pada periode terpilih (Pasal 12 huruf b) — bukan `updatedAt`/nasabah lama yang datanya diedit, seperti yang keliru diimplementasikan semula. Field individu vs. korporasi (Pasal 7) juga dikonfirmasi cocok dengan pemetaan kolom yang sudah dibangun.
  - **Keterbatasan diketahui dan didokumentasikan**: kolom No.NPWP selalu kosong walau Pasal 7 mewajibkannya untuk nasabah Korporasi, karena `customers` belum membedakan individu/korporasi atau menyimpan NPWP per nasabah — perlu diisi manual di file sebelum unggah bila ada nasabah korporasi.
  - Diuji di `server/sipesatExport.test.ts` (6 test, termasuk reproduksi byte-per-byte dari 2 baris contoh resmi PPATK) dan `server/sipesatExportQuery.test.ts` (3 test baru: filter triwulan memakai `createdAt` bukan `updatedAt`, initial data menyertakan nasabah `INACTIVE`/ditutup, error jelas bila ID PJK belum diisi).
- [x] **Fase 2/N pelaporan goAML — generator XML LTKT (LTKTM/LTKTK)**: `shared/goAmlExport.ts` membangun XML LTKT murni dari data yang ada, diverifikasi elemen-per-elemen (termasuk urutan `xs:sequence`, karena XSD sensitif urutan) terhadap `goAMLSchema.xsd`/`goAMLReportDataSchema.xsd` yang diberikan user — 3 kesalahan ditemukan dan diperbaiki sebelum implementasi selesai: (1) tidak ada wrapper `<transactions>` — elemen `<transaction>` langsung sebagai sibling `<report>`; (2) field telepon adalah `tph_number` (angka saja) bukan `phone_number`, `tph_communication_type` adalah `MOB` bukan `M`; (3) urutan elemen `t_person_my_client` yang benar: ...occupation → [identification] → tax_number → tax_reg_number → source_of_wealth (bukan source_of_wealth lebih dulu). Cakupan sengaja sempit: hanya bon **tunai** (`CASH`) berstatus **Selesai** yang memenuhi ambang LTKT (≥Rp500jt), dipetakan sebagai **Multiparty** (nasabah = satu-satunya `party`, role `PJS`) sesuai konfirmasi user sebelumnya — bukan Biparty. Arah SELL→LTKTM (kas masuk)/BUY→LTKTK (kas keluar) mengikuti arah kas Rupiah yang sudah dipakai `completeTransaction`. Satu `<transaction>` per baris mata uang bon (bukan per bon, karena `amount_local`/`foreign_currency` singular per party), akhiran nomor transaksi `-L<n>`. Bon `BANK_TRANSFER` **belum didukung** (butuh institution_code/SWIFT rekening lawan yang belum dicatat sistem — didokumentasikan sebagai gap, bukan ditebak). Field `identifier_type` untuk KTP→`KTP`/Paspor→`PAS` dipetakan langsung dari enum skema; identitas `OTHER` sengaja tidak disertakan sama sekali (skema goAML tidak punya kode "lainnya" generik). `company_profile.goamlRentityId`/`goamlReportingUserCode` (migrasi `0032`) baru untuk header laporan, diisi di Profil Perusahaan. Kartu ekspor baru "Ekspor LTKT goAML (XML)" di Pelaporan Regulator (pilih rentang tanggal + arah kas, unduh XML); transaksi dengan profil nasabah belum lengkap dilewati dan dilaporkan by name di pesan galat, bukan menggagalkan seluruh ekspor. Diuji di `server/goAmlExport.test.ts` (24 test: mapping enum, urutan elemen XSD, escaping, NPWP normalization) dan `server/goAmlLtktExport.test.ts` (7 test: validasi konfigurasi, multi-line/legacy bon, skip nasabah tak lengkap, arah kas, hasil kosong). **Generator LTKM (mencurigakan) BELUM dibangun** — `report_indicator_type` (kode `POLA-xxx`) goAML adalah codebook yang sama sekali berbeda dari indikator TKM internal (`shared/suspiciousTransactionIndicators.ts`), butuh keputusan pemetaan terpisah dengan user sebelum ditulis, bukan ditebak.
- [x] **Fase 1/N pelaporan goAML (LTKT + LTKM)**: tambahkan field KYC yang diwajibkan skema XML goAML (`t_person_my_client`) ke `customers` — `gender`, `nationality` (ISO 2 huruf), `addressType`/`addressCountry`/`addressProvince`/`addressCity`/`addressDistrict`/`addressPostalCode`, `npwp` (migrasi `0031`, semua nullable, murni aditif). Dipelajari langsung dari `goAMLSchema.xsd`/`goAMLReportDataSchema.xsd` yang diberikan user (~12.000 baris XSD gabungan) — dikonfirmasi field mandatory lewat schema, bukan tebakan. Wajib diisi untuk nasabah **baru** (form Tambah/Edit Nasabah, Impor Excel — kolom template baru: JENIS KELAMIN, KEWARGANEGARAAN, JENIS ALAMAT, NEGARA ALAMAT, PROVINSI, KOTA, KECAMATAN, KODE POS, NPWP); nasabah lama tetap jalan dengan kolom `NULL` sampai dilengkapi manual. Konvensi dikonfirmasi ke user: `last_name` goAML dipetakan dari `fullName` utuh (bukan split first/last, karena banyak nama Indonesia satu kata); transaksi tunai valas (BUY/SELL tanpa transfer bank) dipetakan sebagai **Multiparty** dengan nasabah sebagai satu-satunya reported party (sesuai contoh eksplisit di FAQ SIPENDAR untuk transaksi KUPVA), bukan Biparty. Kode konfirmasi dari skema untuk fase berikutnya: `transmode_code` PBVAL=Beli/PJVAL=Jual, `funds_code` UT=Tunai/REK=Rekening, `report_code` LTKTM/LTKTK (kas masuk/keluar) dan LTKM/LTKMT (mencurigakan). **Generator XML LTKT/LTKM itu sendiri BELUM dibangun** — ini baru fase pengumpulan data KYC yang jadi prasyaratnya. Diuji di `server/customerGoamlFields.test.ts` (6 test: gender/nationality/addressCountry/addressType/addressCity wajib, kode negara di-uppercase, field opsional tetap opsional).
- [x] Tambahkan rekap keuangan transaksi (kartu baru di halaman **Laporan**, `reports.transactionRecap`, Controller ke atas) — preset Hari ini/Bulan ini plus rentang tanggal bebas (sudah ada). Menghitung, dari transaksi `COMPLETED` saja: jumlah transaksi, turnover, total beli/jual per mata uang dan kurs rata², serta **estimasi** margin kotor metode rata-rata tertimbang (kurs jual rata² − kurs beli rata²) × volume yang cocok (`min` beli/jual) — dilabeli eksplisit sebagai estimasi, bukan FIFO cost-basis/laba akuntansi penuh, karena sistem tidak melacak lot valuta mana yang terjual dari pembelian mana. Tidak ada perubahan skema (murni agregasi di memori dari `getTransactionReport`). Diuji di `server/transactionRecap.test.ts` (2 test: matematika margin dengan angka bulat terverifikasi manual, dan kasus satu sisi saja menghasilkan null bukan angka salah).
- [x] Tambahkan pencatatan pengeluaran operasional sederhana (`operational_expenses` migrasi `0029`, halaman `/operasional/pengeluaran`, Staff ke atas) — tanggal, kategori terkurasi (`shared/expenseCategories.ts`: sewa, gaji, utilitas, perlengkapan, pemasaran, pemeliharaan, izin/pajak, lainnya), nominal, deskripsi, catatan, bukti pengeluaran opsional (reuse `operational_documents` dengan `ownerType='EXPENSE'`/`documentType='EXPENSE_RECEIPT'`). **Sengaja terpisah total dari sistem transaksi valuta/kas** — tidak menyentuh `exchange_transactions`/`cash_balances`/`bank_accounts` sama sekali. Entri **append-only** (tanpa edit/hapus) untuk jejak audit; koreksi memakai entri baru. Diuji di `server/expenseEntry.test.ts` (4 test: tolak kategori tak dikenal, tolak deskripsi kosong, tolak nominal ≤0, catat entri valid).
- [x] Tegakkan maker-checker pada review transaksi — pembuat transaksi (`exchangeTransactions.tellerUserId`) tidak dapat menjadi peninjau (checker) transaksinya sendiri di `recordReviewAction` (`server/operations.ts`), ditegakkan di server (bukan sekadar UI) sehingga tidak bisa dilewati lewat panggilan API langsung. **Hanya SHAREHOLDER yang boleh melewati aturan ini.** UI (`OperationsDashboard.tsx`) menyembunyikan tombol Tinjau dan menampilkan lencana "Transaksi Anda sendiri" untuk baris milik reviewer sendiri (kecuali reviewer adalah Shareholder). Diuji di `server/makerCheckerReview.test.ts` (4 test: tolak Admin/Controller me-review punya sendiri, izinkan Shareholder bypass, izinkan reviewer lain seperti biasa).
- [x] Perbaiki UX tindak lanjut peninjauan transaksi — sebelumnya tombol "Tinjau" di `OperationsDashboard.tsx` hanya navigasi ke `Monitoring.tsx`, yang tombol "Buka worklist review"-nya navigasi balik ke `OperationsDashboard.tsx` (loop tanpa aksi nyata sama sekali), padahal endpoint server `transactions.review`/`recordReviewAction` sudah ada dan benar sejak lama — tidak ada pemanggil di client. Ditemukan lewat penelusuran usecase langsung sebagai persona reviewer (Direktur), bukan dari test otomatis. Sekarang "Tinjau" membuka dialog di tempat (`OperationsDashboard.tsx`) berisi detail lengkap transaksi (nomor, jenis, nasabah, nilai, rincian baris mata uang, alasan review, alasan ambang underlying, indikator TKM + keterangan bila ditandai mencurigakan — internal-only, tidak bocor ke kwitansi/CSV), catatan keputusan wajib diisi, dan tiga aksi: Setujui/Kembalikan ke teller/Eskalasi, dipetakan langsung ke `transactions.review`. Dibatasi Admin/Controller/Shareholder (`canReview`) sama seperti gating server (`adminProcedure`). Panduan A-Z §5.5 diperbarui. `pnpm check`/`test` (186 lulus, 2 gagal pra-ada tidak terkait)/`build` lulus. **Belum diverifikasi visual di browser dan belum dideploy.**
- [x] Tambahkan indikator eksplisit "memenuhi ambang LTKT" (≥ Rp 500 juta tunai per transaksi atau akumulasi harian) — `assessReviewRequirement` mengembalikan `meetsLtktThreshold` (ambang tetap Rp 500 juta, bukan setting yang bisa diubah operator, karena ini ambang regulasi PPATK), lencana **LTKT** tampil di Daftar Transaksi untuk bon tunai yang memenuhi. Diuji di `server/operations.test.ts`.
- [x] Tambahkan "Auto-isi dari stok" untuk rincian pecahan valuta asing pada transaksi JUAL — isi jumlah yang akan dijual per baris, auto-isi mengisi nilai+jumlah lembar (harga tetap manual), termasuk alur Tukar Pecahan bila komposisi belum pas. Logika auto-isi/tukar-pecahan sekarang dibagikan (`autoFillFromStock`) antara sisi Rupiah (BELI) dan sisi valuta asing (JUAL), tidak diduplikasi.
- [x] Code-splitting bundle client (`client/src/App.tsx`, semua halaman operasional jadi `React.lazy` + satu `Suspense` di `App()`) — chunk utama turun dari ~1,63 MB jadi ~679 KB (kira-kira -58%); setiap halaman jadi chunk kecil sendiri yang dimuat sesuai kebutuhan. `Monitoring` (satu-satunya pemakai `recharts`) sekarang jadi chunk 422 KB terisolasi, hanya diunduh Controller ke atas saat benar-benar membuka halaman itu.
- [x] Deploy putaran 2026-09-01 keempat ke produksi (commit `84ef758`, gabungan 7 commit termasuk migrasi `0026` dan `0027`) — backup bersih, kedua migrasi diterapkan, build sukses dengan chunk terpisah, pm2 stabil, smoke check 200. **Belum diverifikasi visual di browser** (transisi lazy-loading antar halaman) — perlu klik-klik cepat saat sempat.
